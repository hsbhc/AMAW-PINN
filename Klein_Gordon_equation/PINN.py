...
def draw_residual():
    ...
    plt.tight_layout()
    plt.savefig('KG_residual-PINN.pdf')
    plt.show()
...
def draw_epoch_w():
    ...
    plt.plot(s_collect[:, 0], np.exp(-s_collect[:, 3]), 'y-', label='$e^{-s_{it}}$')
    plt.xlabel('$Iters$', fontsize=20)
    plt.ylabel('$\lambda$', fontsize=20)
    plt.legend()
    plt.savefig('KG_S_RAM-AW.pdf', fontsize=20)
    plt.show()
