...
def draw_exact_points(points, N_points=None, show_exact=True):
    ...
    plt.savefig('2dpossion_xnm-WAM.pdf')
    plt.show()
...
def draw_residual():
    ...
    plt.tight_layout()
    plt.savefig('2dpossion_residual-WAM.pdf')
    plt.show()
...
def draw_error():
    ...
    plt.tight_layout()
    plt.savefig('2dpossion_error-WAM.pdf')
    plt.show()
...
def draw_some_t():
    ...
    plt.tight_layout()
    plt.savefig('2dpossion_qie-WAM.pdf')
    plt.show()
...
if __name__ == '__main__':
    ...
    rc3 = -0.5
    rc4 = -0.5
    exact_u = lambda x: np.exp(-500 * ((x[:, [0]] - rc1) ** 2 + (x[:, [1]] - rc2) ** 2)) \
